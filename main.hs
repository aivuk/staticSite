...
menuInLang dirs dict primDir secDir lang = (primMenu ordDirs, secMenu ordDirs)
  where
    ...
    selectPrimState p | p == primDir  =  " class=\"selected\"" 
                      | otherwise     =  ""

    selectSecState p | p == secDir  =  " class=\"selected\"" 
                     | otherwise     =  ""

    primMenu (d:dirs) = ("<li" ++ selectPrimState (fst d) ++ " id=\"" ++ (fst $ linkName $ fst d) ++ "\"><a href=\"#\" onclick=\"javascript:selectMenu('" ++ (fst $ linkName $ fst d) ++ "')\">" ++ (snd $ linkName $ fst d) ++ "</a></li>\n") ++ (pm dirs)

    secMenu (d:dirs) = ("<ul" ++ selectPrimState (fst d) ++ " id=\"" ++ (fst $ linkName $ fst d) ++ "\">\n" ++ (foldl (\s l -> s ++ "<li" ++ selectSecState l ++ ">" ++ slink (fst d) l ++ "</li>\n") "" $ snd d) ++ "</ul>") ++ concatMap sm dirs

    pm = foldl (\s (d,_) -> s ++ "<li id=\"" ++ (fst $ linkName d) ++ "\"><a href=\"#\" onclick=\"javascript:selectMenu('" ++ (fst $ linkName d) ++ "')\">" ++ (snd $ linkName d) ++ "</a></li>\n") ""

    sm = \(prim, sec) -> "<ul id=\"" ++ (fst $ linkName prim) ++ "\"" ++ (foldl (\s d -> s ++ "<li>" ++ slink prim d ++ "</li>\n") "" sec) ++ "</ul>" 

    slink p s | s == secDir  =  snd $ linkName s
              | otherwise    =  "<a href=\"/" ++ url p s ++ "\">" ++ (snd $ linkName s) ++ "</a>"
